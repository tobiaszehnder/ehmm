...
report <- function(segments, model, outdir=".", 
    ...
    #should always pass)
    validateModel(model, strict=FALSE, type=type)
...
reportLMeans <- function(lmeans, statecolors, outdir, prefix){
    paths <- makePath(outdir, prefix, c("lmeans.txt", "lmeans.pdf", "lmeans.png"))
    #write table
    write.table(lmeans, col.names=T, row.names=T, file=paths[1], quote=F, sep="\t")
    #make plots (both pdf and png)
    myheat(t(lmeans), xlab="mark", ylab="state", zlab="mean log count", 
        main="emission", dev=paths[2], col=heatpal,rowColors=statecolors)
    myheat(t(lmeans), xlab="mark", ylab="state", zlab="mean log count", 
        main="emission", dev=paths[3], col=heatpal,rowColors=statecolors)
    #return html
    htmlImgLink(paths[3], paths[1])
}

reportTrans <- function(transP, modelPath, statecolors, outdir, prefix){
    paths <- makePath(outdir, prefix, c("transP.pdf", "transP.png"))
    #plot transition probabilities (both pdf and png)
    myheat(transP, xlab="state to", ylab="state from", zlab="probability",
        main="transition", dev=paths[1], col=heatpal, rowColors=statecolors, 
        colColors=statecolors)
    myheat(transP, xlab="state to", ylab="state from", zlab="probability",
        main="transition", dev=paths[2], col=heatpal, rowColors=statecolors, 
        colColors=statecolors)
    #return html
    htmlImgLink(paths[2], modelPath)
}
...
reportModel <- function(model, labels, statecolors, outdir, prefix){
    ...
    writeModel(model, modelPath, type)
    ...
    if (type == "negmultinom"){
      ...
      htmlSection("1. Model parameters",
        paste(sep="\n", htmlMatToTable(valign="top", matrix(nrow=1, c(lmeans_html, trans_html))))
      )
    }
}
