...
includeOptNames <- c("outdir", "prefix", "annot", "colors", "labels")
...
getSegmentOptions <- function(){
    ...
    see what they are, inside R, type `library(ehmm)` and `?segment`"),
    list(arg="--verbose_kfoots", flag=TRUE),
    list(arg="--endstate", type="integer", vectorial=TRUE,
    help="Feasible end state for learning the model. 
    If an endstate is given, the trainMode is set to viterbi."),
    list(arg="--nbtype", type="character", default="dep",
    help="Negative binomial type (dep, indep)"),
    list(arg="--trainMode", type="character",
...
advancedOpts <- c("tol","verbose","nbtype","init","init.nlev", "rmin", "endstate", "trainMode")
...
segment <- function(counts, regions, nstates=NULL, model=NULL, notrain=FALSE, collapseInitP=FALSE, 
    ...
    if (length(kfootsOpts)>0){
        ...
        if (!all(optNames %in% advancedOpts)) stop("check 'ehmm::advancedOpts' for the list of allowed advanced options")
    }
    kfootsOpts <- c(list(nthreads=nthreads, split4speed=split4speed, framework="HMM", trainMode=trainMode, fix_emisP=fix_emisP, notrain=notrain), kfootsOpts)
    #CHECK ARGUMENTS
    if (!(kfootsOpts$trainMode %in% c("viterbi", "baum-welch")) ){
      stop("invalid --trainMode argument: choose between 'viterbi' and 'baum-welch'")
    }
    if (!is.null(kfootsOpts$endstate) & (kfootsOpts$trainMode != "viterbi")){
      kfootsOpts$trainMode = "viterbi"
      cat("endstates specified: trainMode set to 'viterbi'\n")
    }
    if (!is.list(counts)) { 
        clist <- list(counts)
    } else clist <- counts
    ...
    if (!is.null(model)){
        ...
        dims <- validateModel(model, strict=!notrain, type=kfootsOpts$nbtype)
    ...
    if (notrain){
        ...
        kfootsOpts$verbose <- verbose_kfoots
    }
    ...
    #REORDER STATES (only in negmultinom case and if no model and no endstate is given)
    ...
    list(segments=segms, model=model, posteriors=fit$posteriors, states=fit$clusters,
         ...
         lliks=fit$lliks, escore=fit$escore) #alpha=fit$alpha
}
